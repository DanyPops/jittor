...
describe("model utility ranking", () => {
  ...
  it("exposes ranking through the authenticated operation service without accepting evidence in the request", async () => {
    ...
    const metrics: MetricStore = { record() { throw new Error("unused") }, recordBatch() { throw new Error("unused") }, query() { return [] }, distinctScopes() { return [] }, aggregateUsage() { return [] }, pruneBefore() { return 0 }, checkpoint() {}, close() {} }

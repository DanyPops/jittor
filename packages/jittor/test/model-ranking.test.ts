...
function evidence(
	...
	return {
		model: { provider, model, version: null, canonical: `${provider}/${model}`, aliases: [] },
		dimension,
		value,
...
function local(provider: string, model: string, dimension: string, median: number, confidence = 0.8): ModelMetricAggregate {
	return {
		...
		type: "general",
		dimension,
		unit: dimension === "wall-latency" ? "milliseconds" : "ratio",
		sampleSize: 10,
		median,
		p90: median,
...
function input(overrides: Partial<ModelRankingInput> = {}): ModelRankingInput {
	return {
		...
		externalEvidence: [
			evidence("openai", "gpt-fast", "quality-coding", 0.7),
			evidence("anthropic", "claude-strong", "quality-coding", 0.9),
			evidence("openai", "gpt-fast", "price-input", 1),
			evidence("anthropic", "claude-strong", "price-input", 3),
			evidence("openai", "gpt-fast", "latency", 100),
			evidence("anthropic", "claude-strong", "latency", 300),
		],
...
describe("model utility ranking", () => {
	it("is deterministic with component weights confidence provenance and stable tie breaks", () => {
		...
		expect(first.ranked[0]?.components.map((component) => component.name)).toEqual([
			"quality",
			"cost",
			"latency",
			"context",
		...
		expect(first.ranked.flatMap((item) => item.provenance).some((item) => item.sourceId === "benchmark" && item.revision === "r1")).toBe(
			true,
		);
	});
	...
	it("never adds a candidate from evidence", () => {
		const result = rankModelCandidates(
			input({ externalEvidence: [...input().externalEvidence, evidence("google", "gemini-extra", "quality-coding", 1)] }),
		);
		expect(result.ranked.map((item) => item.identity).sort()).toEqual(
			candidates.map((item) => `${item.provider}/${item.model}:${item.thinking}`).sort(),
		);
	});
	...
	it("ranks against the design domain (Design Arena Elo evidence) the same way it ranks coding", () => {
		const result = rankModelCandidates(
			input({
				...
				type: "general",
				externalEvidence: [
					evidence("openai", "gpt-fast", "quality-design", 1380),
					evidence("anthropic", "claude-strong", "quality-design", 1290),
				],
				localEvidence: [],
			}),
		);
	...
	it("ranks against the new math domain (Artificial Analysis math_index) the same way it ranks coding and design", () => {
		const result = rankModelCandidates(
			input({
				...
				type: "general",
				externalEvidence: [
					evidence("openai", "gpt-fast", "quality-math", 87.2),
					evidence("anthropic", "claude-strong", "quality-math", 62.9),
				],
				localEvidence: [],
			}),
		);
	...
	it("exposes ranking through the authenticated operation service without accepting evidence in the request", async () => {
		...
		const benchmarkStore = {
			...
			latest(sourceId: string) {
				return sourceId === "benchmark"
					? { sourceId, snapshotId: "s1", retrievedAt: 1_000, publishedAt: 1_000, observations: [benchmark] }
					: null;
			},
		};

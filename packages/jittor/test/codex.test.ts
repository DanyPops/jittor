...
describe("experimental Codex subscription telemetry", () => {
	it("calls the official CLI usage path without exposing credentials", async () => {
		...
		const adapter = new CodexSubscriptionTelemetryAdapter({ accessToken: "oauth-secret", accountId: "account-1" }, async (request) => {
		...
		expect(snapshot.metrics.map((metric) => [metric.scope, metric.metric, metric.value])).toContainEqual([
			"codex:primary",
			"used-fraction",
			0.25,
		]);
	...
	it("parses the live primary-only weekly schema without conflating a separate model limit", () => {
		...
		expect(snapshot.additionalLimits[0]).toMatchObject({
	...
	it("parses official response-header updates for default and metered limits", () => {
		...
		expect(updates[0]).toMatchObject({
			limitId: "codex",
			primary: { usedPercent: 30.5, windowSeconds: 18_000 },
			credits: { balance: "9.5" },
		});
		expect(updates[1]).toMatchObject({
			limitId: "codex_bengalfox",
			limitName: "GPT-5.2 Codex Sonic",
			primary: { usedPercent: 12, windowSeconds: 3_600 },
		});
	});
	...
	it("fails closed on schema drift and impossible percentages", () => {
		expect(() => parseCodexUsage({ ...usagePayload, rate_limit: { ...usagePayload.rate_limit, allowed: "yes" } }, 1000)).toThrow(
			"schema changed",
		);
		expect(() =>
			parseCodexUsage(
		...
		).toThrow("used percent");
		expect(() => parseCodexRateLimitHeaders(new Headers({ "x-codex-primary-used-percent": "not-a-number" }), 1000)).toThrow(
			"header schema changed",
		);
	});

	it("reports transport failures without logging or returning OAuth tokens", async () => {
		const adapter = new CodexSubscriptionTelemetryAdapter({ accessToken: "oauth-super-secret", accountId: "account-1" }, async () =>
			response({ error: "denied" }, 401),
		);
		let message = "";
		try {

...
describe("SQLite metric store", () => {
	...
	it("records and queries observations in chronological order", () => {
		...
		store.record({
			...
			unit: "usd",
			observedAt: 2000,
			attributes: { model: "openai/gpt-4.1-mini" },
		});
		store.record({
			source: "openrouter",
			scope: "key:default",
	...
	it("finds distinct scopes for a source within a time window, bounded and alphabetically ordered", () => {
		...
		expect(store.distinctScopes({ source: "pi", since: 0, until: 5_000, limit: 40 })).toEqual([
	...
	it("aggregates usage into (scope, metric, bucket) sums instead of returning raw rows", () => {
		...
		const rows = store.aggregateUsage({
			source: "pi",
			scopes: ["anthropic-vertex:claude-sonnet-5"],
			since: 0,
			until: 10_000,
			bucketSizeMs: 5_000,
			bucketCount: 2,
	...
	it("aggregates a heavy scope's full history exactly, not just a recent tail (the real incident this replaced)", () => {
		...
		for (let index = 0; index < totalRows; index += 1) {
			store.record({
				...
				value: 10,
				unit: "tokens",
				observedAt: Math.floor((index / totalRows) * windowMs),
			});
		}
		...
		const rows = store.aggregateUsage({
			...
			since: 0,
			until: windowMs,
			bucketSizeMs: windowMs / bucketCount,
			bucketCount,
		});

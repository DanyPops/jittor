...
describe("OpenRouter telemetry adapter", () => {
	it("reads official key limits and projects normalized budget metrics", async () => {
		...
		expect(snapshot.metrics.map((metric) => [metric.metric, metric.value, metric.unit])).toContainEqual([
			"remaining-fraction",
			0.4,
			"ratio",
		]);
		expect(snapshot.metrics.find((metric) => metric.metric === "remaining-fraction")?.attributes).toEqual({
			limit: 100,
			remaining: 40,
			reset: "monthly",
		});
	...
	it("normalizes per-response token, reasoning, cache, and cost accounting", () => {
		...
		expect(metrics.map((metric) => [metric.metric, metric.value])).toEqual([
			...
			["total-tokens", 125],
			["reasoning-tokens", 10],
			["cached-read-tokens", 40],
			["cached-write-tokens", 5],
			["cost", 0.004],
			["upstream-cost", 0.003],
		]);
	...
	it("loads canonical model capabilities and pricing", async () => {
		const adapter = new OpenRouterTelemetryAdapter("secret-key", async () =>
			json({
				data: [
					{
	...
	it("looks up generations and gates beta analytics on management capability", async () => {
		...
		const adapter = new OpenRouterTelemetryAdapter("secret-key", async (request) => {
			...
			if (request.url.includes("/generation"))
				return json({ data: { id: "gen-1", total_cost: 0.02, tokens_prompt: 200, tokens_completion: 50 } });
		...
		expect(await adapter.queryAnalytics({ metrics: ["total_usage"] })).toEqual({
			data: [{ model: "openai/gpt-4.1-mini", total_usage: 1.25 }],

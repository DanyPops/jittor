...
describe("Codex failure classification", () => {
	...
	it("separates retryable rate and overload errors from terminal quota and auth errors", () => {
		expect(classifyCodexFailure({ error: { code: "rate_limit_exceeded", message: "try again" } }, { status: 429 })).toMatchObject({
			kind: "rate-limit",
			transient: true,
		});
		expect(classifyCodexFailure({ error: { code: "server_is_overloaded" } }, { status: 503 })).toMatchObject({
			kind: "overload",
			transient: true,
		});
		expect(classifyCodexFailure({ error: { code: "insufficient_quota" } }, { status: 429 })).toMatchObject({
			kind: "quota",
			transient: false,
		});
		expect(classifyCodexFailure({ error: { code: "invalid_api_key" } }, { status: 401 })).toMatchObject({
			kind: "authentication",
			transient: false,
		});
		expect(classifyCodexFailure({ error: { code: "invalid_prompt" } }, { status: 400 })).toMatchObject({
	...
	it("parses bounded Retry-After values and treats malformed payloads as unknown", () => {
		expect(classifyCodexFailure("timeout", { retryAfter: "12" })).toMatchObject({
			kind: "transport",
			transient: true,
			retryAfterMs: 12_000,
		});

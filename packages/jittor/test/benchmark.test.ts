...
describe("benchmark evidence", () => {
	it("normalizes provider identities without merging dated versions", () => {
		...
		expect(normalizeModelIdentity("openai", "gpt-5.4-2026-03-01").canonical).not.toBe(
			normalizeModelIdentity("openai", "gpt-5.4").canonical,
		);
	});
	...
	it("rejects facts without bounded provenance, freshness, confidence, or license", () => {
		...
		expect(() => validateBenchmarkObservation(observation({ provenance: { ...observation().provenance, url: "not-a-url" } }))).toThrow(
			"URL",
		);
		expect(() => validateBenchmarkObservation(observation({ provenance: { ...observation().provenance, confidence: 2 } }))).toThrow(
			"confidence",
		);
		expect(() => validateBenchmarkObservation(observation({ provenance: { ...observation().provenance, retrievedAt: 0 } }))).toThrow(
			"retrieval",
		);
		expect(() => validateBenchmarkObservation(observation({ provenance: { ...observation().provenance, freshUntil: 999 } }))).toThrow(
			"freshness",
		);
		expect(() => validateBenchmarkObservation(observation({ provenance: { ...observation().provenance, license: "" } }))).toThrow(
			"license",
		);
	});
	...
	it("publishes only complete immutable snapshots through the metric-store port", () => {
		...
		metrics.record({
			source: "benchmark:openrouter-models",
			scope: "openai/gpt-broken",
			metric: "price-input",
			value: 1,
			unit: "usd",
			observedAt: 2_000,
			attributes: { snapshotId: "snapshot-incomplete" },
		});
	...
	it("maps bounded OpenRouter model facts with source-specific provenance", async () => {
		...
		const source = new OpenRouterBenchmarkSource(
			async (request) => {
		...
		expect(requests.map((request) => request.url)).toEqual([
			expect.stringContaining("/models?limit="),
			expect.stringContaining("sort=latency-low-to-high"),
			expect.stringContaining("sort=throughput-high-to-low"),
		]);
	...
	it("fetches a curated allowlist of Design Arena categories, tags them one quality-design dimension, and drops OpenRouter-unreachable models", async () => {
		const requests: Request[] = [];
		const source = new OpenRouterDesignArenaSource(
			"api-secret",
			async (request) => {
				requests.push(request);
		...
		expect(requests.map((request) => new URL(request.url).searchParams.get("task_type")).sort()).toEqual([
			"codecategories",
			"dataviz",
			"svg",
			"uicomponent",
	...
	it("tags LMArena arena evidence under its own dimensions, never the AA-scale quality-coding/quality-type-planning ones", async () => {
		const requests: Request[] = [];
		const source = new LmArenaHfSource(
			async (request) => {
				requests.push(request);
				...
				const row =
					config === "webdev"
						...
						: {
								...
								score_ci_upper: 0.147,
								observation_count: 831160,
								session_count: 23549,
								rank: 1,
								category: "overall",
								leaderboard_publish_date: "2026-07-21",
		...
		expect(snapshot.observations.every((item) => item.dimension !== "quality-coding" && item.dimension !== "quality-type-planning")).toBe(
			true,
		);
	...
	it("skips non-overall LMArena rows and fails closed on schema drift", async () => {
		const withNonOverall = new LmArenaHfSource(
			async () =>
				Response.json({
					rows: [
						{
		...
		const malformed = new LmArenaHfSource(
			async () =>
				Response.json({
					rows: [
						{
							...
							row: {
								...
								organization: "openai",
								category: "overall",
								leaderboard_publish_date: "2026-07-21" /* no rating/score */,
							},
							truncated_cells: [],
						},
					],
	...
	it("maps Artificial Analysis's direct API into the SAME dimensions as the OpenRouter passthrough, plus a new quality-math domain and measured latency", async () => {
		const requests: Request[] = [];
		const source = new ArtificialAnalysisDirectSource(
			"aa-secret",
			async (request) => {
				requests.push(request);
				return Response.json({
		...
		expect(snapshot.observations.map((item) => item.dimension).sort()).toEqual([
			"latency",
			"quality-coding",
			"quality-general",
			"quality-math",
	...
	it("exposes typed refresh, status, and query daemon operations", async () => {
		...
		expect(await service.execute("benchmark.query", { sourceId: "openrouter-models" })).toMatchObject({
		...
		const source: BenchmarkSource = {
			...
			async fetch() {
				...
				return {
					...
					retrievedAt: calls,
					observations: [observation({ provenance: { ...observation().provenance, sourceId: this.id, retrievedAt: calls } })],
				};
			},
		};
		const catalog = new BenchmarkCatalog(new MetricBenchmarkStore(new MemoryMetricStore()), [source], {
			clock: () => 10,

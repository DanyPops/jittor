...
const summary: ContextAssessment = {
	...
	compaction: {
		...
		averageCacheReadTokensBetween: 1_000,
...
describe("Jittor CLI context telemetry parity", () => {
	...
	it("exposes stable bounded JSON through the authenticated typed client", async () => {
		...
		const code = await runCli(["context", "--json", "--since", "1000", "--until", "2000"], {
			client: {
				async call(operation: "context.assess", input: { since?: number; until?: number }) {
	...
	it("exposes benchmark refresh and query through independent JSON and human presenters", async () => {
		...
		expect(
			await runCli(["benchmarks", "list", "--source", "openrouter-models", "--json"], {
				client: client as never,
		...
		output.length = 0;
		expect(
			await runCli(["benchmarks", "list", "--source", "openrouter-models"], {
				client: client as never,
				stdout: (line) => output.push(line),
				stderr: (line) => output.push(line),
				systemctl: () => {},
	...
	it("exposes model ranking with explicit candidates and scope authority", async () => {
		...
		const ranking = {
			...
			scopeWarning: "Pi available models are not the exact session scope",
			domain: "coding",
			type: "general",
		...
		expect(calls[0]).toMatchObject({
			...
			input: {
				candidates: [{ provider: "openai", model: "gpt-5.4", thinking: "high" }],
				scopeAuthority: "available-models",
				domain: "coding",
				type: "general",
				budgetPressure: 0.5,
	...
	it("records, queries, and prunes metrics through the typed daemon client with validated flags", async () => {
		...
		expect(
			await runCli(
				["metrics", "record", "--source", "s", "--scope", "sc", "--metric", "m", "--value", "null", "--unit", "count", "--json"],
				deps,
			),
		).toBe(0);
		...
		expect(
			await runCli(
				["metrics", "record", "--source", "s", "--scope", "sc", "--metric", "m", "--value", "1", "--unit", "not-a-real-unit"],
				deps,
			),
		).toBe(2);
	...
	it("applies and clears a router override with an explicit route and optional expiry", async () => {
		...
		expect(calls[0]).toEqual({
			operation: "router.override",
			input: { route: { provider: "openai", model: "gpt-5.4", thinking: "high" }, expiresAt: 5_000 },
		});
		...
		expect(
			await runCli(["router", "available-routes", "--route", "openai/gpt-5.4@high", "--route", "openrouter/x/y@medium", "--json"], deps),
		).toBe(0);
		expect(calls.at(-1)).toEqual({
			...
			input: {
				routes: [
					...
					{ provider: "openrouter", model: "x/y", thinking: "medium" },
				],
			},
		});
		...
		expect(await runCli(["router", "current-route", "--route", "openai/gpt-5.4@high", "--session-id", "session-a", "--json"], deps)).toBe(
			0,
		);
		expect(calls.at(-1)).toEqual({
	...
	it("never prints the daemon bearer token or a provider credential in status, error, or human output", async () => {
		...
		const failing = {
			async call() {
				throw new Error("Jittor daemon is not running; install or start jittor.service");
			},
		};
		const errors: string[] = [];
		expect(
	...
	it("renders actionable human output and rejects invalid bounds", async () => {
		...
		const deps = {
			client: {
				async call(_operation: "context.assess", _input: { since?: number; until?: number }) {
					return summary;
				},
			} as never,
			stdout: (line: string) => output.push(line),

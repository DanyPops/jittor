...
describe("ContextHubCapability", () => {
	it("records a valid contribution and surfaces it as the producer's latest segment", () => {
		...
		expect(hub.contributedSegments()).toEqual([
			{ key: "rules", label: "Active Rules", estimatedTokens: 200, confidence: "exact-cooperative" },
		]);
	});
	...
	it("replaces a producer's segment with its next contribution rather than accumulating stale ones", () => {
		...
		hub.observe(
			contribution({
				sequence: 2,
				segment: { key: "rules", label: "Active Rules", estimatedTokens: 260, confidence: "exact-cooperative" },
			}),
		);
	...
	it("keeps segments from two different producers independently", () => {
		...
		hub.observe(
			contribution({
				producerName: "lector",
				segment: { key: "index", label: "Symbol index", estimatedTokens: 50, confidence: "correlated" },
			}),
		);
		expect(
			hub
	...
	it("drops a duplicate (same producer + sequence) contribution silently", () => {
		...
		hub.observe(
			contribution({
				sequence: 5,
				segment: { key: "rules", label: "Active Rules", estimatedTokens: 999, confidence: "exact-cooperative" },
			}),
		);
...
describe("buildContextReport", () => {
	...
	it("bounds rendered items per segment rather than dumping every one", () => {
		...
		const report = buildContextReport(
			breakdown({ segments: [{ key: "x", label: "X", estimatedTokens: 190, confidence: "audited", items }] }),
		);
...
describe("toolLedgerSegment integration with buildContextReport", () => {
	it("renders a real Pi tool inventory grouped by owning extension", () => {
		...
		const report = buildContextReport(
			breakdown({ totalTokens: 100, contextWindow: 200_000, effectiveBudget: 200_000, segments: [segment] }),
		);

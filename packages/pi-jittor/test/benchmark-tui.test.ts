...
function ranking(count = 2): ModelRankingResult {
	return {
		...
		ranked: Array.from({ length: count }, (_, index) => ({
			...
			utility: 0.9 - index / 100,
			confidence: 0.8,
			components: ["quality", "cost", "latency", "context", "reliability"].map((name) => ({
				name: name as any,
				score: 0.75,
				confidence: 0.8,
				weight: 1,
				evidenceCount: name === "reliability" ? 10 : 2,
				reason: name === "reliability" ? "10 local reliability observations" : "2 external observations",
			})),
			provenance: [
				{
	...
	it("does not expose a selection action for available-model authority", async () => {
		...
		const client = {
			async call(operation: string, input: unknown) {
				...
				return operation === "models.rank" ? ranking() : { observedAt: 1, sources: [] };
			},
		};
